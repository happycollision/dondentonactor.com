...
function makeFindWrapper(start: string[], end: string[] = start) {
	const exps = start.map((open, i) => `${open}([^${end[i]}]*)${end[i]}`).join("|")
...
const link: Replacer = (injectClass) => [
	/\[(.*?)\]\((.*?)\)/g,
	(_, linkText, linkHref) => `<a${injectClass("a")} href="${linkHref}">${linkText}</a>`,
]
...
const emDash: ReplaceArgs = [/([^-])---([^-])/g, (_, before, after) => `${before}&mdash;${after}`]
