...
if (incorrectArgs) helpAndExit("You called the command without the proper arguments. See help:\n\n")
 ...
 * @param str {string|undefined}
 */
const isNotValue = (str) => str.startsWith("-") || str === "" || str === undefined
 ...
 */
const isNotFolder = (pathpart) => !(fs.existsSync(pathpart) && fs.statSync(pathpart).isDirectory())
...
function rimrafContents(directory, options = {}) {
	...
	try {
		...
		const folders = all.filter((x) => fs.statSync(path.join(directory, x)).isDirectory())
		const files = all.filter((x) => !folders.includes(x))
		const unlinkFiles = files.map((filename) => unlink(path.join(directory, filename)))
...
function recursiveCopyFiles(oldFolder, newFolder) {
	...
	const folders = all.filter((x) => fs.statSync(path.join(oldFolder, x)).isDirectory())
	const files = all.filter((x) => !folders.includes(x))
	...
	const copyingFolders = folders.map((subfolder) =>
		recursiveCopyFiles(path.join(oldFolder, subfolder), path.join(newFolder, subfolder)),
	)
